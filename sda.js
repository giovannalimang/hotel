...
function SalvarDados() {
    ...
    // sessionStorage.setItem('quartos.executivo', JSON.stringify(lista_quartos.executivo.map(quarto => Quarto.toJSON(quarto))));
    // sessionStorage.setItem('quartos.executivo_vista_mar', JSON.stringify(lista_quartos.executivo_vista_mar.map(quarto => Quarto.toJSON(quarto))));
    // sessionStorage.setItem('quartos.familia', JSON.stringify(lista_quartos.familia.map(quarto => Quarto.toJSON(quarto))));
    // sessionStorage.setItem('quartos.praiano', JSON.stringify(lista_quartos.praiano.map(quarto => Quarto.toJSON(quarto))));
    // sessionStorage.setItem('quartos.premium', JSON.stringify(lista_quartos.premium.map(quarto => Quarto.toJSON(quarto))));
    // sessionStorage.setItem('quartos.luxo', JSON.stringify(lista_quartos.luxo.map(quarto => Quarto.toJSON(quarto))));
    sessionStorage.setItem('reservas', JSON.stringify(Object.keys(lista_reservas).reduce((acc, tipo) => { acc[tipo] = lista_reservas[tipo].map(reserva => Reserva.toJSON(reserva)); return acc; }, {})));
    sessionStorage.setItem('historico', JSON.stringify(lista_historico.map(historico => Historico.toJSON(historico))));
    let x = [];
    for (let tipo in lista_quartos) {
        x.push(lista_quartos[tipo].map(quarto => Quarto.toJSON(quarto)));
    }

    sessionStorage.setItem('quartos', JSON.stringify(x));
}
function CarregarDados() {
    ...
    lista_quartps = Object.keys(JSON.parse(sessionStorage.getItem('reservas')) || {}).reduce((acc, tipo) => { acc[tipo] = lista_reservas[tipo].map(Reserva.fromJSON); return acc; }, {});
    lista_reservas = Object.keys(JSON.parse(sessionStorage.getItem('reservas')) || {}).reduce((acc, tipo) => { acc[tipo] = lista_reservas[tipo].map(Reserva.fromJSON); return acc; }, {});
